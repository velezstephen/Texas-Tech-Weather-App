...
public class MainNonGPS extends AppCompatActivity implements WeatherServiceCallback,GestureDetector.OnGestureListener{
        ...
        try {
            ...
            if (Math.abs(diffX) > Math.abs(diffY)) {
                    ...
                    if (diffX > 0) {
                        ...
                        if(i<=0){//if been no locations or back to main page
                            ...
                            if(str != null) {//if have locations go to new page
                                ...
                                LocationManager lm = (LocationManager) getSystemService(LOCATION_SERVICE);
                                if (lm.isProviderEnabled(LocationManager.GPS_PROVIDER)){
                                    Intent intent = new Intent(MainNonGPS.this, NewPage.class);
                                    startActivity(intent);
                                }
                                else{
                                    Intent intent= new Intent(MainNonGPS.this,NewPageNonGPS.class);
                                    startActivity(intent);
                                }
                            }
                        }
                ...
                if(diffY>0){
                    ...
                    Intent intent = new Intent(MainNonGPS.this, ForecastPage.class);//go to forecast page on swipe down
                    startActivity(intent);
                }
            }result=true;
        } catch (Exception e) {
    ...
    public void startNotification() {
        ...
        if(!notificationSet) {//set notification
            ...
            Intent intentAlarm = new Intent(MainNonGPS.this, Notify.class);
            AlarmManager alarmManager = (AlarmManager) getSystemService(ALARM_SERVICE);
            PendingIntent result = PendingIntent.getBroadcast(MainNonGPS.this, 0, intentAlarm, PendingIntent.FLAG_UPDATE_CURRENT);
